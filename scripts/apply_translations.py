#!/usr/bin/env python3
"""Apply French translations to all jewelry modules via JSONB field writes.

Run this after module installation/upgrade to translate:
  - Model names (ir.model.name)
  - Field descriptions (ir.model.fields.field_description)
  - Field help texts (ir.model.fields.help)
  - Action names (ir.actions.act_window.name)
  - Menu names (ir.ui.menu.name)
  - Sequence names (ir.sequence.name)
  - View arch strings (ir.ui.view.arch_db)

Usage:
  docker compose exec -T odoo /usr/bin/odoo shell -d bijouterie \\
...
TRANSLATIONS = {
    'ir.model': {
        ...
        'jewelry_transactions.model_supplier_account_line': 'Ligne Compte Fournisseur',
        'jewelry_transactions.model_associate_account': 'Compte Associé',
        'jewelry_transactions.model_associate_account_line': 'Ligne Compte Associé',
        'jewelry_transactions.model_service_order': 'Bon de Service',
        'jewelry_transactions.model_service_order_line': 'Ligne Bon de Service',
    ...
    'ir.model.fields': {
        ...
        'jewelry_transactions.field_casse_melting_line__finished_weight': 'Poids Fini (g)',
        'jewelry_transactions.field_casse_melting_line__pure_weight': 'Poids Pur (g)',
        'jewelry_transactions.field_casse_melting_line__raw_weight': 'Poids Brut (g)',
        'jewelry_transactions.field_casse_melting_line__ticket_id': 'Ticket',
        'jewelry_transactions.field_supplier_account__balance': 'Solde',
        'jewelry_transactions.field_supplier_account__partner_id': 'Fournisseur',
        'jewelry_transactions.field_supplier_account_line__amount': 'Montant',
        'jewelry_transactions.field_supplier_account_line__date': 'Date',
        'jewelry_transactions.field_supplier_account_line__notes': 'Remarques',
        'jewelry_transactions.field_associate_account__balance': 'Solde',
        'jewelry_transactions.field_associate_account__partner_id': 'Associé',
        'jewelry_transactions.field_associate_account_line__amount': 'Montant',
        'jewelry_transactions.field_associate_account_line__date': 'Date',
        'jewelry_transactions.field_associate_account_line__notes': 'Remarques',
        ...
        'jewelry_transactions.field_service_order_line__description': 'Description',
        'jewelry_transactions.field_service_order_line__price_unit': 'Prix Unitaire',
        'jewelry_transactions.field_service_order_line__product_id': 'Article',
        'jewelry_transactions.field_service_order_line__quantity': 'Quantité',
        'jewelry_transactions.field_service_order_line__service_order_id': 'Bon de Service',
        'jewelry_transactions.field_service_order_line__subtotal': 'Sous-total',
    ...
    'ir.sequence': {
        ...
        'jewelry_transactions.seq_barcode': 'Code-barres Ticket',
    },
    'ir.ui.view_arch_db': {
...
def apply_translations(env):
    """Apply all French translations using JSONB field writes (Odoo 17)."""
    for xml_id, value in TRANSLATIONS['ir.model'].items():
        try:
    ...
    for xml_id, value in TRANSLATIONS['ir.model.fields'].items():
        try:
            rec = env.ref(xml_id, raise_if_not_found=False)
            if rec:
    ...
    for xml_id, value in TRANSLATIONS['ir.model.fields.help'].items():
        try:
            rec = env.ref(xml_id, raise_if_not_found=False)
            if rec:
    ...
    for xml_id, value in TRANSLATIONS['ir.actions.act_window'].items():
        try:
            rec = env.ref(xml_id, raise_if_not_found=False)
            if rec:
                rec.with_context(lang='fr_FR').write({'name': value})
    ...
    for xml_id, value in TRANSLATIONS['ir.ui.menu'].items():
        try:
            rec = env.ref(xml_id, raise_if_not_found=False)
            if rec:
                rec.with_context(lang='fr_FR').write({'name': value})
    ...
    for xml_id, value in TRANSLATIONS['ir.sequence'].items():
        try:
            rec = env.ref(xml_id, raise_if_not_found=False)
            if rec:
                rec.with_context(lang='fr_FR').write({'name': value})
    ...
    for view_xml_id, replacements in TRANSLATIONS['ir.ui.view_arch_db'].items():
        try:
